...
export class GithubApp implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Consume GitHub API using GitHub App authentication',
		defaults: {
			name: 'GitHub App',
		},
		usableAsTool: true,
	...
	methods = {
		...
		credentialTest: {
			async githubAppApiTest(
				...
				try {
					// Format the private key (trim and handle escaped newlines)
					...
					const jwtToken = jwt.sign(payload, formattedKey, { algorithm: 'RS256' });
					...
					if (!response.token) {
						return {
							status: 'Error',
							message: 'Failed to obtain installation access token',
						};
					}

					// Test the installation token by making an actual API call
					// This matches the pattern of native GitHub credentials testing with /user endpoint
					const testOptions = {
						method: 'GET' as const,
						headers: {
							'User-Agent': 'n8n',
						...
						uri: `${baseUrl}/installation/repositories`,
						json: true,
					};

					try {
						await this.helpers.request(testOptions);
					} catch (error: any) {
						return {
							status: 'Error',
							message: `Installation token obtained but failed to access API: ${error.message}`,
						};
					}
					...
					return {
						status: 'Error',
						message: error.message || 'Failed to authenticate with GitHub App',
					};
				}
			},
		},
